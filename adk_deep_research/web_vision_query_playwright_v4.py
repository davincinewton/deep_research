...
Critic_Prompt = """You are an image analysing critic. Review the image, user question and answer provided in
previous chats. Focusing on correctness and completeness, provide 1-2 sentences of constructive criticism on how to improve it.
...
Reviser_Prompt="""
You are an image analysing reviser. Review the image, user question and answer provided in previous chats. 
Based on the critics provided in previous chat, output the revised answer in following format:
```json
{
    'isfound': 'yes',
    'answer': 'answer to the user prompt',
...
model_id_c="Devstral-Small-2505-UD-Q4_K_XL" #"koboldcpp/Qwen2.5-VL-7B-Instruct-Q5_K_M"
...
model_id_a="llama-4-scout-17b-16e-instruct" #"14b" #"qwen2.5-vl-32b-instruct" # #"gemma-3-27b-it@q8_0" "gemma-3-27b-it-qat" #"gemma-3-27b-it" #
...
client_c = OpenAI(
    api_key="EMPTY",
    base_url="http://192.168.1.147:8080/v1",
)
client = OpenAI(
    api_key="EMPTY",
...
def inference_with_api(image_byteio, prompt, tmessages:list, model_id, client, sysprompt=None):
...
def inference_with_api_dataurl(image_data:str, prompt, tmessages:list, model_id, client, sysprompt=None):
    if sysprompt is not None:
        event = {
        "role": "system",
        "content": [{"type":"text","text": sysprompt}],
...
def run_analyser(text_input: str, resized_bytes, modelid=model_id_b, clientapi=client_b, is_dataurl=False, sys_prompt= Sys_Prompt):
    messages = ast.literal_eval(Ini_Messages)
    current_prompt = analyser_prompt+text_input
    # print(messages)
    # create
    if is_dataurl:
        response, messages = inference_with_api_dataurl(resized_bytes, current_prompt, messages, model_id=modelid, client=clientapi, sysprompt=sys_prompt)
    else:
        response, messages = inference_with_api(resized_bytes, current_prompt, messages, model_id=modelid, client=clientapi, sysprompt=sys_prompt)
    print(response)
    # print(messages)
    # # critic
...
def run_base64_analyser(text_input: str, resized_bytes, modelid=model_id_b, clientapi=client_b, sys_prompt=Sys_Prompt, is_revised= False):
    messages = [] # ast.literal_eval(Ini_Messages)
    # current_prompt = text_input
    # print(messages)
    # create
    response, messages = inference_with_api_dataurl(resized_bytes, text_input, messages, model_id=modelid, client=clientapi, sysprompt=sys_prompt)
    print(response)
    # print(messages)
    if is_revised:
        # critic
        tprompt = "provide critics for last answer"
        response, messages = inference_with_api(None, tprompt, messages, model_id=modelid, client=clientapi, sysprompt=Critic_Prompt)
        print(response)
        # # print(messages)
        # # revise
        tprompt = "output your revised answer"
        response, messages = inference_with_api(None, tprompt, messages, model_id=modelid, client=clientapi, sysprompt=Reviser_Prompt)
        print(response)   
    # print(messages)
    tmp = response.find("</think>")
    # print(tmp)
...
def run_base64_local_analyser(text_input: str, local_path, modelid=model_id, clientapi=client, sys_prompt=Sys_Prompt, is_revised = True):
    ...
    return run_base64_analyser(text_input, data_uri, modelid=modelid, clientapi=clientapi, sys_prompt=sys_prompt, is_revised=is_revised)
