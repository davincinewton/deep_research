...
MODEL_ID_1  = "Qwen3-30B-A3B-Q8_0" #"gemma-3-27b-it@q8_0" #"qwen3-30b-a3b" #"qwen3-32b" #"gemma-3-27b-it-qat" "a72b4b" 
client_1 = OpenAI(
    api_key='EMPTY',
...
def run_finalizer(text_input: str, modelid=MODEL_ID,clientapi=client):
    ...
    if tmp != -1:
        response = response[tmp+8:]
        # print(response)
    response1st = repair_json(response)   
    # print(messages)
    # # critic
    tprompt = pt.R_F_C
    response, messages = inference_with_api(tprompt, messages, model_id=modelid, client=clientapi)
...
# """
    ...
    for iter in range(2):
        ...
        for tool in tooluse:
            if tool == 1:
                ...
                if iter<1:
                    ...
                    web_results = asyncio.run(process_queries(sagent, ql, max_sources=6))
        ...
        res, res1st  = run_finalizer(all_results_str,modelid=MODEL_ID,clientapi=client)
        # print(tmp)
        try:
            tmp = json.loads(res1st)
            isanswed = tmp.get('isanswered', 'no')
            if isanswed == 'yes':
                # print('answer: ', tmp.get('answer', '...'))
                # print('explanation: ', tmp.get('explanation', '...'))
                tmp = json.loads(res)
                isanswed = tmp.get('isanswered', 'no')
                if isanswed:
                    print('final answer: ', tmp.get('answer', '...'))
                    print('explanation: ', tmp.get('explanation', '...'))            
                    break
                else:
                    print('no answser found this round, try one more time...')
            else:
                tmp = json.loads(res)
                isanswed = tmp.get('isanswered', 'no')
                # print('final answer: ', tmp.get('answer', '...'))
                ...
                if iter<1:
                    if isanswed:
                        print('final answer: ', tmp.get('answer', '...'))
                        print('explanation: ', tmp.get('explanation', '...'))
                        print("However, we will run again to make sure it's the correct answer...")
                    else:
                        print('no answser found this round, try one more time...')
                else:
                    print('no answer found, run finish.')
        except json.JSONDecodeError:
            print("Invalid JSON string")
        # run_finalizer(all_results_str,modelid=MODEL_ID_G,clientapi=client_r)